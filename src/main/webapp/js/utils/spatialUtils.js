...
define([
	 ...
	 * @returns {Object} a pseudo-set -- a map whose keys are the entries 
	 * in an array, and whose values are all true
	 */
	self.makeSetFromArray = function(array){
		return _.object(array, _.map(array, truth));
	};
	/**
	 * 
	 * @param {String} huc
	...
	self.getStatesForHuc = function (huc, context) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				//create a pseudo-set
				var states = self.makeSetFromArray(statesList);
	...
	self.getHucsForStatesAndHuc = function(states, huc, context){
		var deferred = $.Deferred();
